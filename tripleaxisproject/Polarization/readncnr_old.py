...
##        self.header['orient2']=orient2
    ...
    #get first line
        ...
        self.header['count_info']['count_type']=tokenized[8].strip("'").lower()
        ...
        self.header['file_info']['scantype']=tokenized[5].strip("'").lower()
        self.header['file_info']['instrument']=self.header['file_info']['filename'].split('.')[1].lower()
        
        self.header['timestamp']={}
        self.header['timestamp']['month']=tokenized[1].strip("\'").lower()
    ...
    def get_columnheaders_bt7(self,tokenized):
    #get first line
    #    tokenized=get_tokenized_line(myfile)
        self.columndict={}
        #self.columndict['columnlist']=[]
    ...
    #get first line
        ...
        while myFlag:
            ...
            if tokenized[0].lower()=="#Date".lower():
                ...
                self.header['timestamp']['month']=int(date_tokens[1].strip("\'"))
                self.header['timestamp']['day']=int(date_tokens[2].strip("\'"))
                self.header['timestamp']['year']=int(date_tokens[0].strip("\'"))
                self.header['timestamp']['time']=tokenized[2].strip("\'")
                ...
                self.header['timestamp']['Epoch']=timeobj
                #print self.header['timestamp']
            elif tokenized[0].lower()=="#MonoSpacing".lower(): 
                #self.header['dmono']=float(tokenized[1])
                self.header['dspacing']['monochromator_dspacing']=float(tokenized[1])
            elif tokenized[0].lower()=="#AnaSpacing".lower():
                #self.header['dana']=float(tokenized[1]) 
                self.header['dspacing']['analyzer_dspacing']=float(tokenized[1])
            elif tokenized[0].lower()=="#Orient".lower():
                self.header['orient1']={}
                self.header['orient1']['h']=float(tokenized[1])
                self.header['orient1']['k']=float(tokenized[2])
                self.header['orient1']['l']=float(tokenized[3])
                self.header['orient2']={}
                self.header['orient2']['h']=float(tokenized[4])
                self.header['orient2']['k']=float(tokenized[5])
                self.header['orient2']['l']=float(tokenized[6])
            elif tokenized[0].lower()=="#Lattice".lower():
                self.header['lattice']={}
                self.header['lattice']['a']=float(tokenized[1])
                self.header['lattice']['b']=float(tokenized[2])
                self.header['lattice']['c']=float(tokenized[3])
                self.header['lattice']['alpha']=float(tokenized[4])
                self.header['lattice']['beta']=float(tokenized[5])
                self.header['lattice']['gamma']=float(tokenized[6])
            elif tokenized[0].lower()=="#AnalyzerDetectorMode".lower():
                self.header['analyzerdetectormode']=tokenized[2].lower()
            elif tokenized[0].lower()=="#ScanDescr".lower():
                self.header['scan_description']=tokenized[1:]
                print 'reached'
                print self.header['scan_description']
            else:
                currfield=tokenized[0].lower().lower().strip('#')
                self.header[currfield]=(tokenized[1:])    
                
            if tokenized[0]=='#Columns'.lower():
                self.get_columnheaders_bt7(tokenized)
                count =  0
                try:
...
##                    columndict[field].append(tokenized[i])
##        myfile.close()
##        return 
    ...
    def readbuffer(self,myfilestr,lines=N.Inf):
        ...
        if self.instrument in ['bt9','ng5']:
            ...
            if self.header['file_info']['scantype'].lower()=='i':
                print "calling readibuffer"
                self.readiheader(myfile)
            if self.header['file_info']['scantype'].lower()=='b':
                print "calling readbbuffer"
                self.readbheader(myfile)
            if self.header['file_info']['scantype'].lower()=='q':
...
if __name__=='__main__':
    ...
    if 1:
        ...
        mydata=mydatareader.readbuffer(myfilestr,lines=1)
