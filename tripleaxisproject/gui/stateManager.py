...
class DetectorMemento:
    def __init__(self, qmax=0.1, npts=10, beam=None, zmin=None, zmax=None):
        self.qmax = qmax
        self.npts = npts
        self.zmin = zmin
        self.zmax = zmax
    ...
    def toString(self, memento):
        ...
        if memento == None:
            ...
            if not memento.zmin == self.zmin:
                descr += "/Zmin = %g" % self.zmin
            if not memento.zmax == self.zmax:
    ...
    def _onEVT_MODEL_PARS(self, event):
        ...
        if event.disp_changed and len(event.disp)>0:
            if not self.state.model.__class__.__name__ == "Averager2D":
                ...
                new_model = Averager2D()
                under_model = self.state.model.clone()
                print new_model.set_model(under_model)                
