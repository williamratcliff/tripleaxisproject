...
class Devices(object):
	...
	def rate(self):
		...
		mycommand=Rate()
		mycommand.run()
		print 'rate is',mycommand.result
...
#		self.parse_move(arg,relative=True)
	...
	def do_mesh(self,arg,opts=None):
		"""
		...
		usage:qmesh <base scan> <mesh variable> <start> <step> <final>
		ex:mesh qscan -h_c .4 .1 .5
		"""
		args=arg.split()
		devices=Devices()
		parser = argparse.ArgumentParser(description='Do a reciprocal space mesh')	
		parser.add_argument('scan', nargs=1, help='scan to be modified')
		parser.add_argument('-w_i', nargs=3, help='energy transfer initial')
		parser.add_argument('-w_c', nargs=3, help='energy transfer center')
		parser.add_argument('-w_f', nargs=3, help='energy transfer final')
		parser.add_argument('-w_s', nargs=3, help='energy transfer step')
		parser.add_argument('-h_i', nargs=3, help='h initial')
		parser.add_argument('-h_c', nargs=3, help='h center')
		parser.add_argument('-h_f', nargs=3, help='h final')
		parser.add_argument('-h_s', nargs=3, help='h step')
		parser.add_argument('-k_i', nargs=3, help='k initial')
		parser.add_argument('-k_c', nargs=3, help='k center')
		parser.add_argument('-k_f', nargs=3, help='k final')
		parser.add_argument('-k_s', nargs=3, help='k step')
		parser.add_argument('-l_i', nargs=3, help='l initial')
		parser.add_argument('-l_c', nargs=3, help='l center')
		parser.add_argument('-l_f', nargs=3, help='l final')
		parser.add_argument('-l_s', nargs=3, help='l step')
		...
		myargs = parser.parse_args(args)
		if len(args)<5:
			print 'sorry, you have the wrong number of arguments.'
			return
		scan=args[0]
		dev=args[1]
		di=float(args[2])
		ds=float(args[3])
		df=float(args[4])
		#print di,ds,df, list(frange6(di,df,ds))
		for i in list(frange6(di,df,ds)):
			#print 'i',i
			#s='%s %s %3.5f -k_c %3.5f -l_c %3.5f '%(scan,dev,i)
                        s='%s -h_c %3.5f -k_c %3.5f -l_c %3.5f '%(scan,i,i,i)
			self.do_scanop(s)
			self.do_rs(scan)
	...
	def do_howlong(self,arg,opts=None):
		...
		parser.add_argument('scans', nargs='*', help='a bar to be frabbled')
		parser.add_argument('-o', nargs=1, help='overhead')
		myargs = parser.parse_args(arg.split())
		isFile=myargs.f
		isSeconds=myargs.s
		isNow=myargs.now
