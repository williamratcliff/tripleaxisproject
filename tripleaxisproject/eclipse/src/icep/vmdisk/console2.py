...
class Console:
    ...
    def tab(self, event=None):
        ...
        commandlist=['mv','move','mvr']
        motorlist=['a3','a4','a5','utilt']
        try:
            if len(toks)==0:
                self.popup.command_length=0
                self.popup.showMethodCompletionList(commandlist, self.getDisplayPoint())
            elif len(toks)==1: 
                mylist=fnmatch.filter(commandlist,toks[0]+'*')
                self.popup.command_length=len(toks[0])
                self.popup.showMethodCompletionList(mylist, self.getDisplayPoint())
                #text=self.getText()
                #self.resetbuffer()
                ...
                for i in range(1,len(toks)):
                    if i%2!=0:
                        sys.stderr.write(str(i)+' '+toks[i]+'\n')
                        self.popup.command_length=len(toks[i])
                        mylist=fnmatch.filter(motorlist,toks[i]+'*')
                        if mylist:
                            self.popup.showMethodCompletionList(mylist, self.getDisplayPoint())
    ...
    def __initKeyMap(self):
        ...
        keyBindings = [
            ...
            (KeyEvent.VK_TAB, 0, "jython.tab", self.tab),
