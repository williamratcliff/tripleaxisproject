...
def readmeshfiles(mydirectory,myfilebase,myend,eflag='hhl'):
    ...
    for currfile in flist:
        ...
        mydata=readfile(currfile,metadata)
        mon=N.array(mydata.data['monitor'])

        o1=N.array([1,1,1])
        o2=N.array([1,-1,0])
        o3=N.cross(o1,o2)
        h=N.array(mydata.data['h'])
        k=N.array(mydata.data['k'])
        l=N.array(mydata.data['l'])
        ...
        Counts=N.concatenate((Counts,N.array(mydata.data['detector'])*mon0/mon))
    if eflag=='hhl':
