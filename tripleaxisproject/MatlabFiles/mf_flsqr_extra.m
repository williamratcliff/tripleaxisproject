function [p, sig, f, covp,corp,r2,rv]=mf_flsqr_extra(x,y,err,pin,notfixed,func,Q,fcp)
...
f=feval(func,x,Q,p); f=f(:); wt = wt/sum(wt);
...
for iter=1:niter,
    ...
    prt=feval(ldfdp,x,fbest,pprev,dp,func,Q);	% partials of func for x.
%  prt = prt(:);
    r=wt.*(y-fbest);				% error
...
if nargout > 1
	[sig,covp,corp,r2,rv,f] = mf_sig_extra(x,y,wt,p,p*0.01,notfixed,func,f,Q);
	%disp('* Covariance matrix of estimated parameters')
	%disp(covp)
	%disp('* Correlation matrix of estimated parameters ')
	%disp(corp)
end
%disp('mf_flsqr_extra was executed');