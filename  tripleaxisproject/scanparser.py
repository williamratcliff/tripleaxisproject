            ...
            if fields[0]=='q':
                ...
                if len(toks)==3:
                    if toks[-1]=='s':
                        print 'start stop'
                        start=toks[0].split('~')
                        prange['q']['start']=N.array(start).astype('Float64')
                        stop=toks[1].split('~')
                        prange['q']['stop']=N.array(stop).astype('Float64')
                        if npts>1:
                            prange['q']['step']=(prange['q']['stop']-prange['q']['start'])/(npts-1)
                        else:
                            prange['q']['step']=N.zeros(prange['q'][stop].shape,'Float64')
                    elif toks[-1]=='i':
                        print 'initial step'
                        start=toks[0].split('~')
                        prange['q']['start']=N.array(start).astype('Float64')
                        step=toks[1].split('~')
                        prange['q']['step']=N.array(step).astype('Float64')
                        prange['q']['stop']=prange['q']['start']+(npts-1)*prange['q']['step']
                    ...
                    step=toks[1].split('~')
                    prange['q']['step']=N.array(step).astype('Float64')
                    prange['q']['start']=center-(npts-1)/2*prange['q']['step']
                    prange['q']['stop']=center+(npts-1)/2*prange['q']['step']
